...
//============================================
//fj_ltls change for read register

//extern int da9063_read_register(int channel);

extern int da9063_write_register_llp(void);
//============================================
static irqreturn_t da9063_onkey_irq_handler(int irq, void *data)
{
	...
	if (onkey->key_power && !error && (val & DA9063_NONKEY)) {
		...
		printk("<1> ----- KEY_POWER (long) pressed.\n");
	} else {
		input_report_key(onkey->input, KEY_SLEEP, 1);
		input_sync(onkey->input);
		input_report_key(onkey->input, KEY_SLEEP, 0);
		input_sync(onkey->input);
		printk("<1> ----- KEY_POWER (short) pressed.\n");

	}
     //da9063_write_register_llp();//fj_ltls add 

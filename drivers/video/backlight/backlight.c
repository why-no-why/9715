...
//				if (!bd->use_count++) {
				if (bd->use_count<=1) {
					if(brightness_rec==0){
						bd->props.brightness = bd->props.brightness;
					}else{
						bd->props.brightness =  brightness_rec;
					}
					bd->use_count++;
...
//				if (!(--bd->use_count)) {
				if ((--bd->use_count)<=1) {
					brightness_rec=bd->props.brightness;

...
//david_wang for hdmi same addr
	if(fb_id==0){
		addr0_rec=(uint32_t) fbi->fix.smem_start;
	}
	else if(fb_id==2){
		if(b_same_addr){
			fbi->fix.smem_start=addr0_rec;
		}
	}
...
static int mxcfb_option_setup(struct platform_device *pdev, struct fb_info *fbi)
{
	...
	while ((opt = strsep(&options, ",")) != NULL) {
		...
		}else if (!strncmp(opt, "same_addr", 9)) {
			b_same_addr=1;

...
printk(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>in>>>>>\n");
gpio_direction_output(amp_gpio, 1);
...
//david_wang
	...
	amp_gpio = of_get_named_gpio(np, "speaker-amp-gpios", 0);
	ret = gpio_request(amp_gpio, "speaker-amp-gpios");
	if (ret < 0) {
		printk("david_wang request gpio failed, cannot speaker amp controller");
		return ret;
	}
	gpio_direction_output(amp_gpio, 0);
//david_wang_end
